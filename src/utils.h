...
struct utils
{
		 ...
		 * DrawSortedComplete to display the sorted rectangles with a sliding window effect. Otherwise,
		 * it highlights the rectangles being compared or swapped in red, while the rest are drawn in
		 * white.
		 * @tparam Container The type of the container holding the rectangles (e.g.,
		 * std::vector<float>).
		 * @tparam Tracker The type of the tracker used to identify the indices of the rectangles being
		 * compared or swapped (e.g., std::unordered_set<size_t>).
		 * @param rectangles The container of rectangles to be drawn, where each element represents the
		 * height of a rectangle.
		 * @param ScreenWidth The width of the screen for scaling the rectangles.
		 * @param ScreenHeight The height of the screen for scaling the rectangles.
		 * @param trackers The tracker containing the indices of the rectangles being compared or
		 * swapped, which will be highlighted in red.
		 * @param sorted A boolean indicating whether the sorting is complete. If true, the function
		 * will call DrawSortedComplete to display the sorted rectangles with a sliding window effect;
		 * otherwise, it will highlight the rectangles being compared or swapped in red.
		 */
		template <typename Container, typename Tracker>
		static void DrawRectangles(
			const Container& rectangles, int ScreenWidth, int ScreenHeight, const Tracker& trackers,
			bool sorted
		)
		{
				size_t N = rectangles.size();
